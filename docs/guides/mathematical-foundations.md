...
signs of \(q\_{\mathrm{xy}}\) after orientation is correct.
...
\Delta*i =
\begin{bmatrix}
q*{\mathrm{xy},i}^{obs} - q*{\mathrm{xy},i}^{pred} \\
q*{z,i}^{obs} - q\_{z,i}^{pred}
\end{bmatrix}.
\]
...
\((q\_{\mathrm{xy}}, q_z)\). For a horizontal lineout,

\[
I(q*{\mathrm{xy}}) =
\int*{q*{z,min}}^{q*{z,max}}
I(q\_{\mathrm{xy}}, q_z)\,dq_z.
\]
...
\[
I(q*z) =
\int*{q*{\mathrm{xy},min}}^{q*{\mathrm{xy},max}}
I(q*{\mathrm{xy}}, q_z)\,dq*{\mathrm{xy}}.
\]

Arch ROIs use polar coordinates in the \(q\_{\mathrm{xy}}, q_z\) plane:

\[
q*r = \sqrt{q*{\mathrm{xy}}^2 + q*z^2},
\qquad
\chi = \operatorname{atan2}(q_z, q*{\mathrm{xy}}).
\]
...
-\frac{1}{2}
\left(
\frac{(q*{\mathrm{xy}}-\mu*{\mathrm{xy}})^2}{\sigma*{\mathrm{xy}}^2} +
...
The fitted center \((\mu\_{\mathrm{xy}}, \mu_z)\) becomes the peak coordinate
...
|F*{hkl}|^2
W(\mathbf{R})
K*{\sigma}\left(\mathbf{q} - \mathbf{R}\mathbf{G}\_{hkl}\right),
\]

where \(K\_{\sigma}\) is a peak-shape kernel and \(W(\mathbf{R})\) represents the
orientation distribution.

The Ewald-sphere condition can be written as

\[
|\mathbf{k*i} + \mathbf{G}*{hkl}| = |\mathbf{k_i}|.
\]
...
\[
\epsilon*{hkl} =
|\mathbf{k_i} + \mathbf{G}*{hkl}| - |\mathbf{k_i}|.
\]

Reflections with small \(|\epsilon\_{hkl}|\) are close to the Ewald sphere and
...
\[
I*{exp}(\mathbf{q}) \approx a I*{sim}(\mathbf{q}) + b.
\]
...
\[
R(\mathbf{q}) =
I*{exp}(\mathbf{q}) - \left(a I*{sim}(\mathbf{q}) + b\right).
\]
...
- Use detector and correction settings to make \(q\_{\mathrm{xy}}\) symmetry and
